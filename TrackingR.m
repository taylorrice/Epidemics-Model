...
for clock = 0:clockmax
    ...
    for ii = find(personStatus == 2)
        ...
        personStatus(jj) = personStatus(jj)+((rand(1,length(jj))<infectProb)*2);
    end
...
%     end
    ...
    counts = sprintf('%d infected, %d susceptible, %d infected by original', I, S, R);
    axis equal
    axis([0,space,0, space])
    axis manual
    axis on
